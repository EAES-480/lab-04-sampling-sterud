...
> GM GRADE 9.5/10 - Good conceptual understanding and clear writing on seasonality, daily cycles, and sampling strategies. However, there some code issues affected reproducibility and precision.
...
> GM: -0.25 You wrote that you chose `'TA'` but then used `reco` in the code and plots. This inconsistency made several sections confusing. Please be consistent with your chosen variable.
...
> GM: Good points on seasonality, risks of ignoring latent groups, when stratification is essential, and a realistic trade-off.
...
> *The systematic sampling did a better job sampling Population Mean, but with a less decent job minimizing standard deviation. I chose a cycle of 172 days, which is the amount of time from January 1st to June 16th, which is the beginning of the year to the start of summer.*
...
> *Depending on the cluster frame you select, the variance can fluctuate greatly in the dataset because the points in a certain time frame are more likely to be similar to eachother. For cluster sampling to be accurate, you must choose a cluster of values that is near the mean found in the standard population measurement, or includes a high density of those values. This causes lots of variability. For time series data, autocorrelation is relevant because, as mentioned previously, measurements that are close to each other in time are generally more correlated with one another. *

> GM: Good explanation — autocorrelation within days means points in the same cluster are not independent, leading to higher variance.
...
> *Quasi-continuous sampling does not necessarily preserve the 'seasonal' pattern, however it displays a daily pattern within a given month. Quasi-continuous sampling is preferable because you can visualize how a response variable changes from day/night, and the results are more descriptive when taken continuously. Sparse discrete sampling can still display changes in a plot, but less-often the volativity of the variables. Sparse discrete sampling is better used in detecting seasonal variation, not daily.*

> GM: -0.25 Quasi-continuous sampling actually preserves **both** the seasonal and daily patterns well (especially at step=2). It does not only preserve the daily pattern.
...
> *Quasi-continuous sampling gave the best estimate of the population mean because it divides the dataset into small sections that are roughly similar to the standard population mean. It sampled across the whole dataset, and accounts for both diurnal fluctuations as well as seasonal. The cluster(days) sampling has a lower, more preferable SD because of the autocorrelation of linear cluster models which relates similar values amongst specific time frames. This causes less variation within the cluster, but more variation between cluster sets. If I was designing a real EaES study with limited field days, I would propose to stratify the model by seasons, with equal clusters at the hottest and coldest parts of the day for a couple days each month. *
